    ...
    for model_name, id_to_providers in model_bids.items():
        ...
        for model_id, providers in id_to_providers.items():
            ...
            for provider, data in providers.items():
                prices_str = ", ".join(map(str, data["prices"]))
                print(f"    Provider: {provider} -> Prices: {prices_str}")
                print(f"      Total Completed Sessions: {data['total_sessions']}")
                print(f"      Completed Sessions for Model ID {model_id}: {data['model_sessions']}")
