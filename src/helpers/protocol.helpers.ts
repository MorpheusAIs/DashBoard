...
import { IL2Factory } from '@/types/contracts/L2Factory'
import type { InfoDashboardType } from '@/types'

const findMatchingProtocols = (
  l1Pools: IL1Factory.PoolViewStructOutput[],
  l2Pools: IL2Factory.PoolViewStructOutput[],
): [
  IL1Factory.PoolViewStructOutput | null,
  IL2Factory.PoolViewStructOutput | null,
] => {
  const l2PoolMap = new Map<string, IL2Factory.PoolViewStructOutput>(
    l2Pools.map(pool => [pool.protocol, pool]),
  )
...
): Promise<InfoDashboardType.DashboardInfo | null> => {
